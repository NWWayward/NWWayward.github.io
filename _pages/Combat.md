...
## How to Fight
### Attacking
The simplest and most effective way to resolve a conflict is often violence. In some cases, it's the only way. Thus, it behooves any shinobi to be able to, when necessary, incapacitate or kill an opponent. The first step of attacking is to declare what your attack will be, its speed, its cost(s) (Stamina or Chakra Exhaustion) and its accuracy, along with all modifiers the attack (or its parts, such as the costs of the attack) may have, so that the other player knows what you're doing, and can decide how to appropriately respond to it.  Typically, you should do so as follows:
...
Some things add to your "base" damage bonus (either Physical or Ninjutsu): these effects are multiplied if a jutsu or ability states it multiplies your damage bonus (i.e., Great Breakthrough would also double the benefit of any Potent Chakra ranks).  If it does not say it adds to your "base" damage bonus, however, it should be considered **additive** and unaffected by other bonuses.  This means in the above example if you had +2 NDB from utilizing Natural Chakra, it would not be doubled by Great Breakthrough--it would still only give you +2 to your NDB. 

Any effect which lessens your damage bonus, on the other hand, does so to all modifiers equally.

As an example: Using Burning Spear (D-rank Katon) causes you to use your NDB instead of your PDB for your taijutsu attacks with the weapon, and grants you +33%.  If you two-handed it, you get another +33% to your damage bonus (for wielding a two-handed weapon with both hands), in this case your NDB; If you then also had Chakra Flow: Fire activated, you would add an additional 25% of your NDB (which you were already using for it), making it your CHA/15 * (1 + 0.33 + 0.33 + 0.25) = 1.91, not CHA/15 * 1.33 * 1.33 * 1.25.   If you were to add Weapon Focus to that example, you would add it without it being multiplied by any of those effects, as a flat +1 per rank of Weapon Focus applied.

As another example: If you were to have Chakra Strengthening active at X = 3, for +3 PDB, then used a Gouken +10 Dai Dageki attack, you would double your PDB due to Dai Dageki, then add +3 from Chakra Strengthening, then add +2 from Gouken, for (STR/15 * 2 + 3 + 2) multiplied by the attack's speed; it would not be (STR/15 + 3 + 2) * 2.  If you were to also have the two-point unique Bruiser, it would be (STR/15 + RES/30)*2 + 3 + 2.
...
### Delay
Delay is not, properly speaking, an action in and of itself, but rather a quality certain actions have. For an example of how this might be written, an attack could have "Speed 12, Delay 4". This means that while it would be declared normally on your turn (and its cost(s) paid for when you declare it), its effects would take place 4 initiative counts later. The action may be aborted any time up to the last IC of the delay.
...
Taking the hit is the glorious move the stronger person or sensei uses to protect their weaker ally! You can use this to protect one other person from any given attack; you can't Take the Hit for multiple people at once against the same attack, sadly, and in such a situation must pick who you wish to try and protect.  

First, you must be able to reach distance 0 with your target as part of your movement with this action, based on its final Speed; if you can't close to range 0 of them, you are too far away and can't protect them in time (but see Shunshin as a method to get close to someone you are far away from).

You make a Dodge roll (which is still not a 'Dodge action', to be clear) with a +5 bonus; if successful, you've leapt in the way of an attack aimed at your ally. If you fail, your ally takes 20% of the damage per point you failed by. (For example: If you fail by 2 points, you'd take 60% damage, and your ally 40% damage). If you were Blocking as a Variable Speed Action when you used this, you may block as long as you were successful by 1 point or more. You may also use interrupts to defend yourself, up to a maximum Speed of how many points your Take The Hit roll beat the opponent's accuracy; you can't dodge, but you could parry, or use a ninjutsu-based defense that raises a protective wall.
